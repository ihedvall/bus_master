...
namespace bus {
wxBEGIN_EVENT_TABLE(UnknownDestinationDialog, wxDialog)
  EVT_UPDATE_UI(wxID_SAVE, UnknownDestinationDialog::OnUpdateSave)
  EVT_BUTTON(wxID_SAVE, UnknownDestinationDialog::OnSave)
  EVT_TEXT(kIdSave, UnknownDestinationDialog::OnNameChange)
wxEND_EVENT_TABLE()

UnknownDestinationDialog::UnknownDestinationDialog(wxWindow *parent)
...
void UnknownDestinationDialog::SetInvalidNames(
    std::vector<std::string>& invalid_names) {
  invalid_names = std::move(invalid_names);
}

void UnknownDestinationDialog::SetDestination( const IDestination& destination) {
  name_ = destination.Name();
  description_ = destination.Description();

  TransferDataToWindow();
}

bool UnknownDestinationDialog::GetDestination(IDestination& destination) {
  TransferDataFromWindow();
  bool modified = false;
  if (destination.Name() != name_.ToStdString()) {
    destination.Name(name_.ToStdString());
    modified = true;
  }
  if (destination.Description() != description_.ToStdString()) {
    destination.Description(description_.ToStdString());
    modified = true;
  }
  return modified;
}

void UnknownDestinationDialog::OnSave(wxCommandEvent &event) {
...
void UnknownDestinationDialog::OnUpdateSave(wxUpdateUIEvent &event) {
  event.Enable(!name_.IsEmpty());
}

void UnknownDestinationDialog::OnNameChange(wxCommandEvent&) {
  name_ctrl_->TransferDataFromWindow();
}

bool UnknownDestinationDialog::TransferDataToWindow() {
  return wxWindowBase::TransferDataToWindow();
}

bool UnknownDestinationDialog::TransferDataFromWindow() {  // Fix the exe_picker and combo boxes
...
bool UnknownDestinationDialog::IsValidName() const {
